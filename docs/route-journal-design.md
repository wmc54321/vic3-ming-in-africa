...
国家选择界面的“目标”页另增第六项 `objective_mgn_two_chinas`，只列出大明与大清。该入口不复制路线机制，而是通过单一最终子目标把现有 `je_mgn_two_chinas` 挂入目标系统；日志达成臣服或统一结局时，目标随之完成。国家特色日志悬停说明使用专门的 `je_mgn_two_chinas_lobby` 双语文本，避免把游戏内 `_reason` 文本误当作大厅说明键。
...
- `MDC`：内地经略公司，覆盖大清开局领土范围（包括开局由大清直接管辖但保持未整合的拉萨、阿里及东喜马拉雅原西藏部分），并扩展到喜马拉雅、库页岛与琉球；主流文化为汉；首都偏好北京。琉球在公司设立时默认随该范围划入。
...
- 政体彩蛋亦已实现 `law_mgn_national_tutelage`（国民训政制）：法团国家或社会君主制下的大明、大清或合法中华中央政府可制定；它保留一党制行为，追加军队与小市民政治力量，并开启轻量“训政时期”日志。玩家可在非竞选期间宣布至少持续两年的全国戒严，把下次选举延期到正常时间线之外；解严后六个月恢复选举，且两年内不得重新戒严。日志无进度条和周期事件，废法时会清理戒严状态。当前 1.13 实机已确认选举延期与存读档链路；完整机制见 `docs/implementation-map.md`，设计取舍见 `docs/design-plan.md` 的“国民训政制彩蛋”。
- 两项法律另有固定、非随机的州名彩蛋：按当前法律与固定 `STATE_*` 地理州映射24个中国/非洲代表州，控制单方领土时只改本国实际州实例，统一双方领土后自然扩展到完整表；表外州与海外新征服地保留原名。下埃及在革命委员会制下称“中央苏区”，国民训政制下称“兴中”；作为中华君主国家首都且未采用两法时称“应天行在”。州易主、迁都和废法会刷新并恢复原版动态地名，城市“应天府”与港口“靖海港”不变。完整映射见 `docs/design-plan.md`，脚本入口与维护规则见 `docs/implementation-map.md`。
...
31. 设立 `MDC` 后，大清开局领土（包括未整合的拉萨、阿里及东喜马拉雅原西藏部分）以及喜马拉雅、库页岛、琉球、吉尔吉斯、伊犁、图瓦中的可用州转移给公司，公司成为特许公司附庸。
...
47. 法团国家与社会君主制都能制定国民训政制；其他治理原则只能预览灰显法律。法律保留一党制行为，并显示军队、小市民政治力量各 `+15%` 与政府规模合法性 `+2`。
48. 国民训政制生效后出现“训政时期”；正在竞选时不能宣布戒严，非竞选期宣布后获得“动员戡乱时期”，下一次选举不再按原周期发生。
49. 戒严未满两年时不能解严；满两年、无内战及酝酿中的革命时可以解严，六个月后正常举行选举，随后两年不能重新戒严。
50. 戒严期间存档并重载后，戒严修正、按钮状态及被延期的选举日期保持不变。
51. 戒严期间废除国民训政制时，日志失效并清理戒严修正与所有相关变量；若新法律仍允许选举，则六个月后举行选举。
52. 1836 大明开局时，下埃及州名显示“应天行在”，城市仍为“应天府”、港口仍为“靖海港”；迁都离开下埃及后，“应天行在”恢复为原版适用名称，重新迁回应天且仍为君主制/社会君主制时再次出现。
53. 制定革命委员会制后，当前拥有的24个入表州按固定革命表改名，下埃及优先显示“中央苏区”；开新局或重新制定时同一 `STATE_*` 不产生不同结果。
54. 制定国民训政制后，当前拥有的24个入表州按固定训政表改名，下埃及优先显示“兴中”；从革命委员会制切换后不残留旧表名称。
55. 废除两项特色法律后，只有带模组标记的州恢复名称；台湾、外满洲、黑龙江、黄金海岸、刚果、南喀麦隆和德兰士瓦重新采用当前原版文化/所有者动态名称，其他事件或 hub 专名不被重置。
56. 大明、大清仅控制各自开局领土时只修改本国实际州实例；一方控制双方领土后同一固定表自然覆盖两边；双方分别采用两项不同法律时，边界两侧同时显示各自主人的法律表。
57. 分割州与州易主时只修改新主人实际拥有的州实例；入表州转给不符合条件的国家后恢复原版名称，新征服的表外州始终保留原名。
58. 在改名状态下存档并重载后，州名与模组标记保持；关闭动态命名游戏规则时不应用特色州名，重新使用正常动态命名规则的新局能恢复本功能。
59. 简体中文与英文环境均显示完整49个名称 key，不出现裸 key；英文拼音专名和制度意译与固定中文表逐项对应。
60. 月度兜底只对大明、大清、当前合法中华中央政府、特色法律使用者或仍持有改名标记州的国家工作；正常运行一年不出现反复闪名、无关州被重置或显著月初卡顿。

国民训政制实测记录：

- 2026-08-15，用户在 Victoria 3 1.13 中完成第 47—51 项实机检查；当前版本接受 `set_next_election_date = "9999.1.1"`，戒严状态与延期日期可跨存读档保持，解严和废法后的选举恢复链路按设计工作。
- 这项结论只适用于当前支持版本。游戏更新后仍须重跑第 47—51 项，不能因为已有一次通过记录就跳过发布门槛。

固定政体地名实测记录：

- 2026-08-15，用户已进入 Victoria 3 1.13 检查并接受当前州名的游戏内显示，未报告可见命名或本地化错误；记为表现检查通过。
- 用户没有逐项报告第 52—60 项的全部路径，因此迁都往返、两法切换、双方异法并存、分割州易主、原版动态名称恢复、存读档、关闭动态命名、英文环境与一年性能观察仍保留为发布前回归项。
