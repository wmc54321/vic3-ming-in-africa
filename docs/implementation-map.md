...
- `building_mgn_imperial_academy`: the buildable Imperial Academy of the Four Seas in Lower Egypt. Construction may begin only while Lower Egypt is Ming's capital, but the completed academy remains active after a later capital move. It costs 1,200 construction and, at full employment, consumes 10 Paper while employing 500 Academics, 300 Clerks, and 200 Bureaucrats. Two fixed production-method groups make scope explicit in the standard UI: `Four Quarters Dispatches (Nationwide)` grants `+1%` Education Access across Ming, while `Yingtian Statecraft (Lower Egypt)` grants `+2%` Peasant Education Access and `+10%` Qualifications only in Lower Egypt. All three effects scale with employment. Distinct education modifier types prevent the standard tooltip from merging nationwide and local values into a misleading single total. The nationwide value uses a whole percentage point because this modifier type's standard UI has zero decimal places. Setting text about dispatching academy graduates remains in the building description because Victoria 3 1.13's generated production-method tooltip does not consume arbitrary `_desc` localization. The academy deliberately grants no Innovation, research speed, Authority, Legitimacy, Prestige, or output bonus. Its custom DXT5 building icon depicts a fortified vermilion-and-gold Ming academy on the Nile; the full-resolution PNG source is retained under `assets/source/` for future art revisions.
