...
- 新增下埃及可建造的大明版紫禁城奇观“应天府紫禁城”，图标、生产方式和效果复用北京紫禁城；统一后迁都南京时另有“南京紫禁城”可由玩家自行建造。下埃及与北京分别可建设“四海国子监”和“京师国子监”，下埃及与广东分别可建设“靖海市舶总行”和“广州十三行”；这些成对奇观只绑定地理位置，不绑定国家、文化、政体或首都，并允许同一国家同时拥有与叠加。
...
- 埃及是非洲开局国家，但原版还持有黎凡特、阿达纳部分地区和克里特；初版将埃及在非洲外的残余领土、人口和建筑归属转给奥斯曼，避免 `EGY` 作为开局国家残留。阿达纳须合并原版埃及与奥斯曼的分裂州作用域；克里特须同名覆盖南欧人口与建筑历史，不能用后加载的重复州块追加。
...
- `common/history/buildings/01_south_europe.txt` 与 `common/history/pops/01_south_europe.txt`：生成当前原版的完整同名副本，只把克里特的 `region_state:EGY` 及建筑所有权改为 `TUR`。实机日志证明另建后加载的重复 `s:STATE_CRETE` 块无法解析 `region_state:TUR`，因此不采用聚焦追加文件。
...
- 下埃及可建造“四海国子监”，北京可建造“京师国子监”，两者开局均不直接建成。它们不检查建造者国家或首都位置，可以被任何当地所有者建设或在征服后继承，并允许同一国家同时拥有。每座需要 1200 建设量；满编时消耗 10 纸张，雇佣 500 学者、300 文员和 200 官僚，并按就业比例提供全国普通教育机会 `+1%`，以及仅限所在地的农民教育机会 `+2%` 与人口资质增长 `+10%`。
- 广东可建造“广州十三行”，下埃及可建造“靖海市舶总行”，两者开局均不直接建成。它们同样只绑定地理位置，任何当地所有者均可建设或继承，同一国家可以同时拥有。每座需要 1000 建设量；满编时消耗 5 纸张和 5 商船，雇佣 500 店主、300 文员和 200 官僚，并按就业比例提供所在地贸易容量 `+25%`、贸易优势 `+10%` 与全国影响力 `+5%`。
...
- `building_mgn_imperial_academy` 保留为“四海国子监”的旧存档兼容键，固定在下埃及；新增 `building_beijing_imperial_academy`“京师国子监”，固定在北京。两者不再检查国家标签、文化、政体或首都位置：任何当地所有者都可建设，征服后由新所有者继续受益，同一国家可以同时拥有并叠加全国收益。每座建设量为 1200，满编消耗 10 纸张并雇佣 500 学者、300 文员和 200 官僚。第一轮实测发现原定全国农民教育机会 `+5%` 会造成过快的识字率增长，因此每座最终采用全国普通教育机会 `+1%`、所在地农民教育机会 `+2%`、所在地人口资质增长 `+10%`；同时拥有两座时全国教育机会合计 `+2%`，本地效果仍分别留在下埃及和北京。四海国子监使用`四方遣教（全国）`与`应天经世（下埃及）`，京师国子监使用`学政颁行（全国）`与`京师经世（北京）`。全国与本地采用不同修正键，避免原版 UI 合并显示；作用域直接写入生产方式名称，完整特色介绍保留在建筑说明中，不为此覆盖全局 GUI。两座均不附加创新力、科研速度、权威、正统性、威望或产出加成，并与各地宫城不互斥。
- `building_guangzhou_thirteen_factories`“广州十三行”固定在广东，`building_jinghai_maritime_exchange`“靖海市舶总行”固定在下埃及。两者沿用国子监已经采用的地缘而非国家绑定：不检查国家、文化、政体或首都，征服后继续存在，同一所有者可同时拥有。每座建设量为 1000，满编消耗 5 纸张和 5 商船，雇佣 500 店主、300 文员和 200 官僚；本地生产方式按就业比例提供贸易容量 `+25%` 与贸易优势 `+10%`，全国生产方式提供影响力 `+5%`。广州使用“粤海通商（广东）／十三行商路（全国）”，靖海使用“靖海互市（下埃及）／四海商路（全国）”，以免 UI 混淆作用域。两座共用广州十三行滨水商馆风格图标；两所国子监也统一换用无热带元素的国子监建筑图标。四座建筑的新地缘建造条件与两套新图标仍需游戏内确认。
...
已批准的经济公司定名为“坤舆矿务总局”（Kunyu Directorate of Mines）。“坤舆”承接大明在非洲重新丈量天下、经营四方矿脉的世界观；“总局”强调它是朝廷特许、官督商办色彩浓厚的近代矿务机构，而不是另一块拥有国土的特许公司属国。

首版确定规则：

- 仅大明可设立，优先以德兰士瓦、开普殖民地或下埃及为总部。
- 基础产业覆盖煤矿、铁矿、硫矿、铅矿和金矿。五类产业明显宽于原版多数历史公司，因此公司繁荣只提供 `+5%` 铸币收入，不再附加矿山吞吐、税收或建设增益。
- 玩家需要先在南部非洲的已整合州建成 5 级煤矿或铁矿。这个门槛足够早、目标清楚，又不是开局无条件赠送；金矿不参与解锁，避免随机发现金矿左右玩法节奏。
- 产业特许可从钢铁厂和炸药厂中选择一个。两者分别承接铁矿和硫矿的下游链条，但沿用原版“一家公司只能拥有一个产业特许”的机会成本，不同时白送两条产业链。
- 优质产品采用“龙印精铁”（Dragon-Seal Bar Iron），基础商品仍是铁。它表现总局以统一炉号、尺寸和龙印验记建立的优质条铁标准，避免把“优质金锭”误做成可自由交易的新商品：原版黄金不是普通市场商品，名产金锭既难形成正常市场反馈，也会使主题从工业化滑向直接财政奖励。
- 龙印精铁沿用原版历史公司专属名产生命周期：总局首次达到 100 繁荣度时由引擎自动启用，启用后即使繁荣度下降也永久保持，不另建日志、变量或手动授予效果。公司 `+5%` 铸币收入则是独立的繁荣修正，在 100 繁荣度启用、跌破 75 时关闭。名产与产业特许依赖《Charters of Commerce》功能，公司本体不设置强制 DLC 门槛。
- `prestige_bonus = 0.1` 与原版瑞典条铁、俄罗斯铁一致，是名产系统使用的系数，不应在文档或本地化中写成“国家总威望 +10%”。龙印精铁自动继承名产的消费偏好、出口优势、投入品吞吐和生产排行榜威望，不额外重复添加静态修正。

美术上采用最早生成的公司徽记与铁锭构图。后续曾尝试把龙爪严格改成五爪并补齐铁锭第四枚角饰，但多轮重绘破坏了原稿的比例、爪部形态或整体风格，因此这些候选全部否决；首版宁可保留观感更好的原稿，精确的局部手工修订留待以后另行批准。

明确不采用的替代方案包括：以“名贵铁”作为最终名称（过于泛化，缺少制度与品牌辨识度）、把黄金设为名产基础商品、要求先发现或扩建金矿、为名产另做一套日志或 `prestige_goods_trigger`，以及在五类矿业覆盖之外继续叠加高额矿山吞吐。
...
- 历史废案：德兰士瓦只在事件/日志里称为`金山矿区`、不实际改州名。当前固定政体地名表已经取代该判断：革命委员会制下称“继续革命”，国民训政制下称“庄敬”，其他政体仍保留原名。
- 历史废案：下埃及曾考虑 `西京`、`埃及应天府` 或 `尼罗京`。当前最终方案为君主制首都“应天行在”、革命委员会制“中央苏区”、国民训政制“兴中”，不再使用这些备选名。
...
- 开局西藏不再作为大清附庸存在：拉萨、阿里及东喜马拉雅原属西藏的省份由生成器转给 `CHI`，并逐一设置 `state_type = unincorporated`。人口、宗教、藏族 homeland 与 `TIB` 可释放标签保持原样；中亚建筑和人口历史的对应作用域同步从 `TIB` 改为 `CHI`，避免所有权改变后漏载内容。
...
- `law_mgn_national_tutelage`（国民训政制）是原版一党制的权力分配变体，只能与法团国家或社会君主制治理原则并存。它沿用革命委员会制的中华中央政府范围：大明、大清或当前合法中华中央政府可见，军阀、附属国、特许公司及仅因汉文化主流而命中的其他国家均排除；不要求先赢得两中华斗争，AI 不主动制定，也不能强加给附属国。
- 数值完整保留原版一党制的权威、选票合法性、意识形态一致性、投票基数、执政集团吸引力、唯一合法党和选举信心行为；另加政府规模合法性 `+2`，军队与小市民政治力量各 `+15%`。军队代表革命军、党军与军政统一，小市民代表基层党部、公务员、警察、地方商会和城市职员。未采用知识分子加成，以免路线更像技术官僚制或单纯的五院专家政府。
- 法律生效时添加内政日志“训政时期”。日志不设进度条、阶段资源或周期事件，只提供“宣布全国戒严”与“解除全国戒严”两个互斥按钮。宣布戒严要求当前不在选举活动期间且距上次解严至少两年；它把下一次选举日期推迟到 `9999.1.1`，施加至少两年的“动员戡乱时期”，提供 `+100` 权威与 `-15%` 政治运动激进度。
- 解严要求戒严至少持续两年、国家不处于内战、没有正在酝酿的革命且不在选举活动期间；解严后六个月举行选举，并有两年不得重新戒严。废除国民训政制会使日志失效，并自动清除戒严修正和变量；若新的权力分配法律仍有选举，则安排六个月后的选举。
- 黑色幽默采用一本正经的制度文本：“非常时期预计结束日期另行公告，历次检讨均确认其仍有继续之必要”以及“戒严并非停止训政，而是为了排除妨碍国民接受训政的一切因素”。不增加人物互动、军事攻防、政府吞吐、专属旗帜或动态国名，避免盖过两中华主线。

技术边界与废案：

- 本地 1.13 脚本只确认 `set_next_election_date`、`call_election` 和 `in_election_campaign`，未找到取消正在进行选举的接口，因此禁止在竞选期间宣布戒严，不伪造零选票或覆盖全局选举触发器。
- 远期日期 `9999.1.1` 用于把定期选举推到正常游戏时间线之外。2026-08-15 的 Victoria 3 1.13 用户实测已确认宣告、存读档、解严、废法四条路径按设计工作。该结论随支持版本维护；若未来引擎不再接受或保存该日期，应停止发布这一效果并重新研究，不能用不可靠的表面修正冒充暂停选举。
- 废案包括军队与知识分子政治力量、单纯政府行政吞吐、“训政尚未完成”纯被动减激进度、军政—训政—宪政大型阶段日志，以及还政于民结局。最终采用军队与小市民加成，并把唯一特色玩法压缩为可开关的戒严状态。

#### 固定政体地名彩蛋

- 革命委员会制与国民训政制各有一套固定州名。名称只由当前法律和 `STATE_*` 地理州决定，不看开局归属，不使用随机数；大明、大清各守一方、互相蚕食或由一方统一双方领土时，同一地块在同一法律下始终得到同一名称。双方采用不同法律时，边界两侧可同时出现两套政治地理。
- 当前 1836 开局大明 92 州、大清 46 州均进入刷新检查，但第一版只为 24 个代表州设置专名：中国方向与非洲方向各 12 个，其余州固定保留原名。表外征服地同样保留原名，避免把少数口号随机循环到全球。分割州只修改制定国实际拥有的州实例。
- 中国方向固定表：北京“东方红/中正”、南京“立新/中山”、广东“向阳/逸仙”、河南“红旗/民权”、湖南“韶山/民族”、江西“井冈山/建国”、西安“延安/西京”、贵州“遵义/力行”、台湾“解放/光复”、山东“东风/自强”、外满洲“反修/安边”、黑龙江“卫东/兴安”；斜线前为革命委员会制，斜线后为国民训政制。
- 非洲方向固定表：下埃及“中央苏区/兴中”、中埃及“团结/三民”、上埃及“工农/五权”、西奈“反帝/国民”、东撒哈拉“长征/训政”、黄金海岸“红光/实业”、刚果“大联合/建设”、南喀麦隆“三结合/博爱”、肯尼亚“自力更生/民生”、桑给巴尔“友谊/民享”、开普殖民地“解放/成功”、德兰士瓦“继续革命/庄敬”。
- 下埃及另有君主制基线：大明、大清或当前合法中华中央政府以此为首都，实行君主制或社会君主制且未采用两项特色一党制法律时，州名为“应天行在”。“行在”表现朝廷以非洲应天府为事实京师、同时仍以恢复中华故土为未竟使命。优先级固定为“中央苏区”或“兴中”高于“应天行在”；迁都、废法或易主后重新计算。
- 第一版只修改州名，不修改城市、港口、矿区、农区或林区 hub，因此下埃及的“应天府”“靖海港”始终保留。废法时只恢复带有模组州名标记的州，并在重置基础州名后重新调用原版动态地名效果；台湾、外满洲、黑龙江及部分非洲州因原版已有文化/国家动态地名，不能只做裸 `reset_state_name`。
- 英文本地化对韶山、井冈山、延安、中山、中正等专名采用拼音，对“中央苏区”“反帝”“三结合”等直接制度梗采用简洁意译；两种语言均为完整固定表，不使用占位文本。
- 2026-08-15 用户已在 Victoria 3 1.13 中进入游戏检查并接受当前政体州名的实际显示。该记录确认功能能够加载且当前可见命名表现无异常报告；迁都、双方采用不同特色法律、分割州易主、关闭动态命名及完整中英文存读档仍按发布矩阵分别回归，不把一次视觉检查扩大解释为全部生命周期路径均已证明。

#### 远期法律变体构想池（未批准、未排期）

> **状态说明：以下内容仅为历史推演与设计构想，目前没有实现计划。** 它们不是待办事项，不属于任何已批准版本或二期范围，也不应写入 `docs/implementation-map.md` 作为现有功能。若未来要实现其中任何一项，必须由用户重新明确批准，并在动工前重新核对当时版本的原版法律、反向引用、事件条件、公司法律继承和动态国名触发；不得因为本节存在就自动实施。

总体方向是让法律变体回答“大明如何选官、教育、征税、赈济、维持地方秩序并把海外领土转化为国家内部”等问题，而不是只给原版法律更换中华名称。构想中的变体应当是有明确取舍的侧向制度，不做全面强于母法的终局神法；特许公司原则上只继承对应原版母法，不自动获得中央国家专属变体。

建议优先级仅代表未来若重新讨论时的设计价值，不表示开发顺序：

| 构想优先级 | 暂定名称 | 原版母法 | 定位 |
| --- | --- | --- | --- |
| S | 科举官僚制 | `law_appointed_bureaucrats` | 以科举、共同经典教育和官员考成为基础的文官国家 |
| S | 一条鞭法 | `law_per_capita_based_taxation` | 把田赋、丁役和杂项折并为较标准化的货币税负 |
| S | 经略建省制 | `law_colonial_resettlement` | 放慢单纯移民扩张，强化未整合领土的行政化和建省 |
| A | 书院私塾制 | `law_private_schools` | 廉价、深入乡村但普及程度有限的地方教育网络 |
| A | 天下公学 | `law_public_schools` | 面向万民、重普及而较少依赖强制同化的公共教育 |
| A | 常平义仓制 | `law_poor_laws` | 以储粮、平粜和备荒为核心，现金福利较弱的赈济制度 |
| A | 保甲制 | `law_local_police` | 户籍互保、夜巡、乡村治安与有限地方动员 |
| A | 礼教宽容 | `law_freedom_of_conscience` | 以国家礼制维持公共秩序，同时容纳多种宗教共同体 |
| A | 厂卫制度 | `law_secret_police` | 大明专属的宫廷耳目、诏狱与官僚监察体系 |
| B | 朝贡互市 | `law_mercantilism` | 将朝贡、港口互市与印度洋商业结合的国家贸易秩序 |
| B | 士绅公议 | `law_oligarchy` 或 `law_landed_voting` | 专制与现代议会之间的士绅、商会和官僚有限代议路线 |
| C | 卫所屯田制 | `law_peasant_levies` | 世袭军户、驻防屯田和军政合一的历史遗产路线 |

##### 教育与内部安全

**书院私塾制**：建议作为私立学校的侧向变体。它以书院、私塾、寺观经师、会馆义学和地方捐学组成教育网络，并与现有普通州教士人口代表的寺观、祠堂、私塾、经师和谱牒网络相呼应。初步数值构想是把原版私立学校的机构成本优惠从 `-40%` 收窄到约 `-35%`，降低财富对教育机会的贡献，改为每级机构给少量农民教育机会，并把原版偏向知识分子的政治增益拆分给知识分子与儒学士。它应当比普通私立学校更深入乡村，但仍明显不如公立教育普及。备选名包括“书院教育”“乡学书院制”；暂以“书院私塾制”最直观。

**天下公学**：建议作为公立学校的侧向变体。其目标不是进一步堆高同化，而是让非洲中华、海东中华、交南中华等桥梁文化进入同一套公共教育和行政语言。初步数值构想是把每级教育机会从原版约 `+12.5%` 提高到约 `+15%`，把同化加成从约 `+12.5%` 降到约 `+7.5%`，保留教育热忱与识字增长，以“更广泛教育、较温和整合”区别于原版公立学校。备选名“万民公学”与“天朝万民”绑定过深，“同文公学”又像专门学校，因此暂以“天下公学”为首选。

**厂卫制度**：建议作为秘密警察的大明本体专属变体，不向大清、军阀、附属国、特许公司或仅有汉文化主流的国家开放。初步数值构想是降低原版秘密警察对军官政治力量的集中增益，转移一部分给官僚；定点压制略强，降低整个政治运动激进度的能力略弱，保留分离主义控制、顺从度、政变抵抗和“制造意外”互动。它不再增加诏狱、廷杖、抄家等新人物按钮，以免与革命委员会的“批斗”重复。备选名“锦衣卫”只指一个机构，“缉事衙门”辨识度较低，因此法律暂名“厂卫制度”。

**保甲制**：建议放在警务法律组，作为地方警察变体，而不是秘密警察或国内安全变体。它代表户籍互保、夜巡、乡村治安和地方动员，核心是行政触角而非政治镇压。初步构想是降低地方警察给予地主的政治力量，把少量增益分给儒学士或小市民；治安效果略弱，但附带很小的税收能力或征兵动员增益。不得同时给予显著治安、税收和征兵强化，以免成为三合一最优法。备选名“里甲保甲制”能同时表达税役与治安，但概念较拥挤。

##### 官僚、财政与地方整合

**科举官僚制**：建议作为任命官僚变体，是本构想池中最值得未来重新讨论的一项。它表现科举取士、共同经典教育、地区名额和官员考成所形成的士大夫官僚国家。初步数值构想是把普通任命官僚的税收能力与官僚政治力量略微下调，换取官僚资格增长、政府行政机构雇佣或行政效率的小幅增益，并给予儒学士有限政治力量。天下秩序应强烈支持，海外改革支持但可更偏好普通现代任命制，海贸开放中立。备选名“考成官僚制”更有明代制度味道但不够直观；“考试院制”适合现代共和彩蛋，不适合通用历史法律。

**一条鞭法**：建议作为人头税制变体，以土地和人口核算、折银征收和合并杂项负担表现财政标准化。它应降低人头税比重、提高土地税比重、保留少量所得税，并以较低行政或征管成本作为交换，而不是提高总税率上限。地主应反对，官僚和儒学士支持，商人可因货币化与税目统一而轻微支持。它向大明、大清和合法中华中央政府开放，不要求赢得“两中华”斗争。

**经略建省制**：建议作为殖民安置变体，用来解决“非洲既是大明本土基础、法律却永远把扩张理解成殖民”的叙事问题。初步构想是降低殖民增长或未整合地区移民吸引力，换取未整合地区税收能力、制造业条件或州整合速度的小幅提高；不获得殖民剥削的种植园、矿业吞吐优势，也不增加原住民压迫。其路线区别应是：殖民安置擅长迁入人口，殖民剥削擅长榨取资源，经略建省擅长缓慢转化为正式行政区域。备选名“改土归流”会把所有非洲地方社会误写成土司，“内地化政策”又过于现代直白，因此暂用“经略建省制”。特许公司不能制定此法，因为它的制度意义正是逐步结束公司式和殖民式治理。

##### 赈济、宗教与次要路线

**常平义仓制**：建议作为济贫法变体，把官仓、义仓、社仓、储粮备荒和平抑粮价发展成传统国家的社会保障制度。初步构想是现金福利低于普通济贫法，但食物安全、灾荒死亡控制更强，并给予儒学士少量政治力量。不得把它做成没有腐败、运输和地方执行成本的完美救济体系。备选名“社仓赈济”偏基层，“备荒仓政”制度味较弱。

**礼教宽容**：建议作为信仰自由变体。它不是把儒学简单定义成排他国教，而是要求佛教、伊斯兰教、基督教和地方宗教共同体服从国家礼制与行政登记，同时不追求全面改宗。初步构想是给儒学士少量政治力量和权威，降低改宗速度，并改善无共同宗教特质人口的最低接受度；不得允许对已受接纳人口进行强制改宗。备选名“礼教为本”制度性更强但没有直接表达宽容，“诸教从俗”又容易被理解成地方自治。

**朝贡互市**：可作为重商主义变体，把朝贡、港口互市、特许贸易和印度洋商业结合。设想中关税或纯贸易优势略低于重商主义，换取影响力、外交杠杆或少量公司特许能力。但它与现有特许公司路线和人民资本合作制高度重叠，因此即使将来重开讨论也应低优先处理。

**士绅公议**：可作为寡头制或土地投票变体，表现书院议政、地方绅董、商会和官僚共同构成的有限代议政治。它能提供专制与现代普选之间的中华中间路线，但权力分配法律会影响动态国名、统一条件、政党、选举和大量事件，当前又已有革命委员会变体，因此兼容面和维护成本过高。

**卫所屯田制**：可作为农民征召兵变体，表现世袭军户、驻防屯田、粮仓和军政合一的历史遗产。它不适合当前 1836 大明主线：大明已经拥有职业化军政与线列步兵，开局 392 营也已确认来自非洲兵营归属而非军制法律。把卫所做强会鼓励玩家倒退军制，做弱又缺乏实现价值，因此仅保留为历史背景构想。

##### 暂不建议扩展的法律组

- 不设计大明专属奴隶制变体；开局奴隶贸易已经是需要严肃处理的暴力制度，不再用专属机制增加趣味化或美化风险。
- 不优先设计妇女权利变体；没有配套人物、事件和社会史时，单独换一个中华名称缺乏实质内容。
- 不新增移民法律变体；现有移民政策外交博弈已经为该法律组提供足够多的专属玩法。
- 不新增君主制变体；它会牵动动态国号、旗帜、统一条件、外交与公司脚本，维护面过大。
- 不继续增加末期经济制度；人民资本合作制已经占据该路线最醒目的制度彩蛋位置。
- 不增加更多人物处置型法律；厂卫的“制造意外”和革命委员会的“批斗”已经足够。

若未来只重新批准四项，建议优先重新研究“科举官僚制、一条鞭法、经略建省制、常平义仓制”。它们与教育、厂卫构想可以组成“传统帝国的科举—书院—保甲体系，经过财政标准化、公共教育和建省行政走向不同现代路线”的连续演化；此优先建议同样不是当前排期。
